...
const T = (n) => typeof n == "object" && n !== null && !Array.isArray(n), B = (n, i, v = []) => {
  ...
  return i.every((y) => y in n) && Object.keys(n).every((y) => f.has(y));
...
function xt(n) {
  ...
  for (const [d, u] of Object.entries(n.elements)) {
    ...
    const C = ke(u.evidence);
    f[d] = Object.freeze({ id: d, kind: u.kind, label: u.label, evidence: C, diagnosticIds: Object.freeze([...u.diagnosticIds]), ...u.parentId === void 0 ? {} : { parentId: u.parentId }, ...u.declaration === void 0 ? {} : { declaration: u.declaration }, ...u.limitations === void 0 ? {} : { limitations: Object.freeze([...u.limitations]) }, ...u.source === void 0 ? {} : { source: Ue(u.source) } });
  }
  ...
  for (const d of Object.values(f)) {
    ...
    let C = d;
    for (; C && !y.has(C.id); ) {
      if (u.has(C.id)) throw new x(`Element nesting forms a cycle: ${C.id}`);
      u.add(C.id), C = C.parentId ? f[C.parentId] : void 0;
    }
    for (const k of u) y.add(k);
  }
  ...
  for (const d of n.diagnostics) {
    ...
    w.add(d.id), g.push(Object.freeze({ id: d.id, code: d.code, severity: d.severity, message: d.message, ...d.elementId === void 0 ? {} : { elementId: d.elementId }, ...d.source === void 0 ? {} : { source: Ue(d.source) } }));
  }
  for (const d of Object.values(f)) for (const u of d.diagnosticIds ?? []) if (!w.has(u)) throw new x(`Unknown diagnostic: ${u}`);
  for (const d of g) if (d.elementId && !f[d.elementId]) throw new x(`Unknown diagnostic element: ${d.elementId}`);
  return Object.freeze({ protocolVersion: 1, run: Object.freeze({ id: n.run.id, completedAt: n.run.completedAt, root: Object.freeze({ path: n.run.root.path }), tool: Object.freeze({ name: n.run.tool.name, version: n.run.tool.version }), outcome: n.run.outcome }), svg: n.svg, elements: Object.freeze(f), edges: Object.freeze(b), diagnostics: Object.freeze(g), folded: Object.freeze(v) });
}
...
function Lt(n) {
  ...
  for (; ; ) {
    ...
    const y = i.indexOf("{", f);
    if (y < 0) return { base: i.slice(0, f), darkBody: v };
    const b = i.slice(f, y).trim();
    let I = 0, g = y;
    for (; g < i.length; g += 1)
      if (i[g] === "{") I += 1;
      else if (i[g] === "}" && --I === 0) break;
    Ct.test(b) && (v += `${i.slice(y + 1, g)}
`), i = i.slice(0, f) + i.slice(Math.min(g + 1, i.length));
  }
}
function Ot(n, i) {
  if (!n.trim() || n.length > 32768) return;
  const { base: v, darkBody: f } = Lt(n), y = i ? `${v}
${f}` : v, b = /\s*([^{}]+)\{([^{}]*)\}/gy, I = [];
  let g = 0;
  for (; g < y.length; ) {
    b.lastIndex = g;
    const w = b.exec(y);
    if (!w) return y.slice(g).trim() === "" ? I : void 0;
    g = b.lastIndex;
    const d = w[1], u = w[2];
    if (d === void 0 || u === void 0) return;
    const C = d.split(",").map((D) => D.trim());
    if (!C.every((D) => kt.test(D))) return;
    const k = [];
    ...
    k.length && I.push({ selectors: C, declarations: k });
  }
  return I;
}
function Ke(n, i = {}) {
  ...
  for (const y of [...f.querySelectorAll("*")]) {
    if (y.localName.toLowerCase() === "style") {
      const b = Ot(y.textContent ?? "", v);
      if (b) for (const I of b) for (const g of I.selectors)
        for (const w of [...f.documentElement.querySelectorAll(g)])
          for (const [d, u] of I.declarations) w.setAttribute(d, u);
      y.remove();
      continue;
    }
    if (Et.has(y.localName.toLowerCase())) {
      y.remove();
      continue;
    }
    for (const b of [...y.attributes]) {
      const I = b.name.toLowerCase(), g = b.value.trim().toLowerCase(), w = /url\s*\(\s*['"]?(?:https?:|\/\/|data:|javascript:|file:)/i.test(g);
      (I.startsWith("on") || I === "style" || w || St.has(I) && g !== "" && !g.startsWith("#")) && y.removeAttribute(b.name);
    }
  }
...
function zt(n, i, v = {}) {
  const f = v.margin ?? 24, y = v.minZoom ?? 0.2, b = v.maxZoom ?? 4, I = Math.max(1, n.width - f * 2), g = Math.max(1, n.height - f * 2), w = Math.max(1, i.width), d = Math.max(1, i.height), u = Math.min(b, Math.max(y, Math.min(I / w, g / d)));
  return {
    zoom: u,
...
function Ft(n, i, v = []) {
  n.innerHTML = Vt;
  const f = n.querySelector(".ply-vis"), y = f.querySelector(".ply-canvas"), b = f.querySelector(".ply-stage"), I = f.querySelector(".ply-legend-panel"), g = f.querySelector(".ply-tooltip"), w = f.querySelector(".ply-context-menu"), d = f.querySelector(".ply-inspector"), u = f.querySelector(".ply-inspector-toggle"), C = f.querySelector(".ply-workspace"), k = f.querySelector(".ply-status"), D = f.querySelector(".ply-toolbar [data-evidence-filters]"), W = f.querySelector(".ply-options"), q = f.querySelector(".ply-options-toggle"), M = f.querySelector(".ply-breadcrumbs"), U = f.querySelector(".ply-provenance");
  let a = Dt(), h, ne = /* @__PURE__ */ new Map(), oe, G = Ee(), H;
  ...
  }, et = () => h ? Object.values(h.elements).filter((e) => !a.focusedId || e.id === a.focusedId || ue(e, a.focusedId, h.elements)) : [];
  function V() {
    d.hidden = a.detailsHidden, C.classList.toggle("is-inspector-hidden", a.detailsHidden);
  ...
  function se() {
    if (I.replaceChildren(), !h || !a.legendVisible) {
      I.hidden = !0;
      return;
    }
    const e = (l) => [...b.querySelectorAll(l)].some((m) => !m.closest("[hidden]")), t = /* @__PURE__ */ new Set();
    for (const l of b.querySelectorAll("[data-state]")) {
      if (l.closest("[hidden]")) continue;
      const m = l.dataset.state;
      m in $e && t.add(m);
    }
    const o = Object.keys(h.elements).length > 0;
    !t.size && !o && b.querySelector("svg") && t.add("declared");
    const s = document.createElement("h2");
    s.textContent = "Legend";
    const r = document.createElement("ul"), c = (l, m, p) => {
      const E = document.createElement("li"), A = document.createElement("span");
      A.className = "ply-legend-swatch", A.setAttribute("aria-hidden", "true"), p.state && (A.dataset.state = p.state), p.symbol && (A.dataset.symbol = p.symbol), p.symbol === "trusted" && (A.textContent = "⛉"), p.symbol === "decision" && (A.textContent = "#");
      const P = document.createElement("span"), de = document.createElement("strong");
      de.textContent = l;
      const ae = document.createElement("span");
      ae.textContent = m, P.append(de, ae), E.append(A, P), r.append(E);
    };
    ...
    e(".ceiling-tested, .ceiling-fuzzed, .ceiling-bounded, .ceiling-proved") && c("Grey depth", "Darker means stronger checks promised", { symbol: "ceiling" }), e(".ceiling-unclaimed") && c("Hatched fill", "Nothing here promises a check", { symbol: "unclaimed" }), e(".fn-chip-box-synth") && c("Violet fill", "Machine-written from its contract", { symbol: "synth" }), e(".strict-notch") && c("Strict request", "Asks for errors, not warnings", { symbol: "strict" }), e(".fn-shield") && c("Trusted claim", "Human-attested, not machine-checked", { symbol: "trusted" }), e(".unresolved-pin, .registry-pin") && c("Open decision", "A question still needs an answer", { symbol: "decision" }), e(".edge-call") && c("Call", "Solid arrow", { symbol: "call" }), e(".edge-flow") && c("Data flow", "Dashed arrow", { symbol: "flow" }), e(".edge-entry") && c("External entry", "Dashed arrow from outside", { symbol: "entry" }), e(".deny-rule") && c("Forbidden call", "This call is not allowed", { symbol: "denied" }), e(".architecture-scope") && c("Module boundaries", "References between modules in one crate", { symbol: "architecture" }), e(".acceptance") && c("Application acceptance", "Finite production-path tests", { symbol: "acceptance" }), I.append(s, r), I.hidden = !1;
  }
  function nt(e) {
    const t = b.querySelector(".verdict-strip-text")?.textContent?.trim(), o = t?.startsWith("Declaration view") === !0, s = t?.startsWith("Evidence view") === !0, r = Object.values(e.elements);
    if (o || !s && e.run.tool.version === "render") return { text: "Promises only — no run has checked this yet, so nothing here can ever be green." };
    if (!s && r.length > 0 && r.every((A) => Oe(A) === "declared")) return {
      text: "This older drawing shows declarations only; it does not record whether it came from render or verification.",
      title: `Artifact ${e.run.id}`
    };
    const m = `Showing a run completed ${new Date(e.run.completedAt).toLocaleString()}.`, p = e.run.tool.version;
    return J !== void 0 && /^[0-9a-f]{64}$/.test(p) && /^[0-9a-f]{64}$/.test(J) && p !== J ? {
      text: `${m} Ply itself has changed since this run, so nothing here would be carried forward — every check would run again.`,
      title: `Run ${e.run.id}
Ran by ${p}
Installed ${J}`
    } : { text: m, title: `Run ${e.run.id}` };
  }
  function ue(e, t, o) {
  ...
  function ot(e) {
    let t = 0, o = e;
    for (const s of ce(e, h?.elements ?? {})) {
  ...
  function it(e) {
    let t = 0;
    for (const o of ce(e, h?.elements ?? {}))
  ...
  function st() {
    ...
    const e = [];
    for (const o of ce(a.focusedId ? h.elements[a.focusedId] : void 0, h.elements)) {
      if (!o.parentId) break;
      e.unshift(o);
    }
  ...
  function Y(e) {
    ...
    if (t.textContent = e?.label ?? "Details", d.append(t), !e || !h) {
      const p = document.createElement("p");
      p.textContent = "Select an item to inspect its declaration and evidence.", d.append(p);
      return;
    }
    if ("fromId" in e) {
      d.append(z("Type", [e.kind])), d.append(z("From", [h.elements[e.fromId].label])), d.append(z("To", [h.elements[e.toId].label]));
      return;
    }
...
`).filter(Boolean);
    ...
    const c = new Map(h.diagnostics.map((p) => [p.id, p])), m = o.diagnosticIds.map((p) => c.get(p)).filter((p) => p !== void 0).map((p) => `${p.code} — ${p.severity}: ${p.message}`);
    if (d.append(z("Diagnostics", m.length ? m : ["No diagnostics supplied."])), d.append(rt(h.run)), o.source) {
  ...
  function me() {
    if (!(!h || !a.selectedId))
      return ne.get(a.selectedId) ?? h.elements[a.selectedId];
  }
  function De(e) {
  ...
  function S() {
    O && Me(O), _(), O = void 0, g.hidden = !0, g.replaceChildren();
  }
  function lt(e, t) {
    if (!h) return [];
    const o = [`${e.kind} · Verdict: ${e.evidence.verdict}`];
    e.evidence.statuses.length && o.push(`Statuses: ${e.evidence.statuses.join(", ")}`);
    const s = Object.entries(e.evidence).filter(([l, m]) => !["verdict", "statuses"].includes(l) && m !== !1 && m !== void 0).map(([l, m]) => `${l}: ${typeof m == "string" ? m : JSON.stringify(m)}`);
    o.push(...s), o.push(...(e.limitations ?? []).map((l) => `Limitation: ${l}`));
    const r = new Map(h.diagnostics.map((l) => [l.id, l]));
    for (const l of e.diagnosticIds) {
      const m = r.get(l);
      m && o.push(`${m.code} — ${m.severity}: ${m.message}`);
    }
  ...
  function ft(e, t) {
    const o = y.getBoundingClientRect(), s = 8;
    g.style.maxHeight = `${Math.max(0, o.height - s * 2)}px`;
    const r = 12, c = g.offsetWidth, l = g.offsetHeight, m = Math.max(s, o.width - c - s), p = Math.max(s, o.height - l - s), E = e - o.left + r, A = t - o.top + r, P = A + l <= o.height - s ? A : t - o.top - l - r;
    g.style.left = `${Math.min(m, Math.max(s, E))}px`, g.style.top = `${Math.min(p, Math.max(s, P))}px`;
  }
  function ze(e, t, o) {
    ...
    if (r) {
      ...
      p.textContent = r.label, l.textContent = De(r), g.replaceChildren(p, l);
    } else if (s) {
      const p = document.createElement("strong");
      p.textContent = s.label, l.textContent = lt(s, e).join(`
`), g.replaceChildren(p, l);
    } else
      l.textContent = c, g.replaceChildren(l);
    g.hidden = !1, at(e);
    const m = e.getBoundingClientRect();
    ft(t ?? m.left + m.width / 2, o ?? m.bottom);
  }
  function Ne(e, t, o) {
  ...
  function pt(e, t, o) {
    const s = y.getBoundingClientRect(), r = 8, c = Math.max(r, s.width - e.offsetWidth - r), l = Math.max(r, s.height - e.offsetHeight - r);
  ...
  function N() {
    ...
    w.hidden = !0, w.replaceChildren(), le = void 0, e && (t?.isConnected ? t : y).focus();
  }
  function je() {
  ...
  function ut(e) {
    ...
    if (s && r.push({ label: `Zoom into ${s.label}`, run: () => Z(s.id) }), a.focusedId && r.push({ label: "Back to Workspace", run: () => Z(void 0) }), h) {
      const l = new Map(h.diagnostics.map((m) => [m.id, m]));
      for (const m of fe ? s?.diagnosticIds ?? [] : []) {
        const p = l.get(m)?.code;
  ...
  }), y.addEventListener("contextmenu", ut);
  const qe = (e) => {
  ...
  function j() {
    if (!h) return;
    gt();
    const e = [...b.querySelectorAll("[data-element-id], [data-ply-id]")], t = a.focusedId ? h.elements[a.focusedId] : void 0;
    for (const r of e) {
      const c = r.dataset.elementId ?? r.dataset.plyId ?? "", l = h.elements[c], m = te(r);
      if (m) {
        r.removeAttribute("hidden"), r.setAttribute("role", "button"), r.setAttribute("aria-label", De(m)), r.classList.toggle("is-selected", m.id === a.selectedId);
        continue;
      }
      if (!l) {
      ...
      const p = Oe(l), E = p === "declared" || a.overlays[p], A = t ? ue(t, l.id, h.elements) : !1, P = !a.focusedId || l.id === a.focusedId || ue(l, a.focusedId, h.elements) || A, de = H === void 0 || A || ot(l) <= Te();
      r.toggleAttribute("hidden", !P || !de || !E && !A);
  ...
  function ht(e) {
    X(), h = e, ne = new Map(e.edges.map((r) => [r.id, r])), $({ runId: e.run.id, selectedId: void 0, focusedId: void 0, detailsHidden: !0, zoom: 1, panX: 0, panY: 0 }, !1);
    const t = Object.keys(e.elements).length > 0;
    D.hidden = !t, M.hidden = !t, u.hidden = !t, t || Le(!0, !1), V(), S(), N(), H = void 0, ye(e.svg), y.dataset.empty = "false";
    const o = y.querySelector(".ply-empty");
    o && o.remove(), F(), j(), Y(me());
    const s = nt(e);
  ...
  function yt(e) {
    ...
    for (const s of b.querySelectorAll("[data-element-id], [data-ply-id]")) {
      ...
      const m = c.width / l.width, p = c.height / l.height;
      if (![m, p].every(Number.isFinite)) continue;
      let E;
      try {
        if (E = s.animate([
          { transform: `translate(${(c.left - l.left) / o}px, ${(c.top - l.top) / o}px) scale(${m}, ${p})`, transformOrigin: "0 0" },
  ...
  function vt() {
    ...
    if (Number.isFinite(e)) {
      if (a.focusedId) {
        const t = h.elements[a.focusedId];
  ...
  function re(e) {
    const t = h && (ne.get(e) ?? h.elements[e]);
  ...
  function Z(e) {
    e && !h?.elements[e] || (e && !h.elements[e].parentId && (e = void 0), S(), N(), $({ focusedId: e, selectedId: e, detailsHidden: !e }), V(), j(), Y(e ? h?.elements[e] : void 0), be());
  }
  function bt() {
    const e = y.getBoundingClientRect(), o = (a.selectedId ? [...b.querySelectorAll("[data-element-id], [data-ply-id]")].find((s) => R(s)?.id === a.selectedId) : void 0)?.getBoundingClientRect();
  ...
  function be(e = !0) {
    ...
    if (!t) return;
    const o = a.focusedId ? [...b.querySelectorAll("[data-element-id], [data-ply-id]")].find((m) => ee(m)?.id === a.focusedId) : t;
    if (!o) return;
    ...
    $(zt({ width: y.clientWidth, height: y.clientHeight }, l)), F(), j();
  }
  f.querySelector('[aria-label="Zoom in"]').addEventListener("click", () => ve(a.zoom * 1.2)), f.querySelector('[aria-label="Zoom out"]').addEventListener("click", () => ve(a.zoom / 1.2)), f.querySelector('[aria-label="Fit canvas"]').addEventListener("click", () => be()), u.addEventListener("click", () => Le(!a.detailsHidden)), f.querySelectorAll("[data-overlay]").forEach((e) => e.addEventListener("change", () => {
    ...
    const t = Q(e.target);
    if (g.hidden || S(), !a.hoverTooltips || !t) {
      _();
      return;
    }
    ...
    const t = Q(e.target);
    !t || e.relatedTarget instanceof Node && (t.contains(e.relatedTarget) || g.contains(e.relatedTarget)) || t.contains(document.activeElement) || S();
    ...
    S();
  }, { passive: !0 }), g.addEventListener("pointerdown", (e) => e.stopPropagation()), y.addEventListener("wheel", (e) => {
    e.preventDefault();
    const t = y.getBoundingClientRect();
    ve(a.zoom * Math.exp(-e.deltaY * 2e-3), { x: e.clientX - t.left, y: e.clientY - t.top });
  }, { passive: !1 }), y.addEventListener("pointerdown", (e) => {
    ...
    if (!(!L.moved && Math.hypot(t, o) < 3)) {
      if (!L.moved) {
        L.moved = !0, y.classList.add("is-panning"), S(), I.hidden = !0;
        try {
          y.setPointerCapture(L.pointerId);
        } catch {
        }
      }
  ...
  const we = () => {
    if (!L) return;
    const e = L.moved;
    e && (Ae = performance.now() + 250, Ce()), L = void 0, y.classList.remove("is-panning"), e && se();
  };
  y.addEventListener("pointerup", we), y.addEventListener("pointercancel", we), y.addEventListener("lostpointercapture", we), y.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && !g.hidden) {
      e.preventDefault(), S();
      return;
    }
    const t = [...b.querySelectorAll("[data-element-id], [data-ply-id]")], o = new Set(t.filter((c) => !c.hasAttribute("hidden") && te(c)).map((c) => c.dataset.elementId ?? c.dataset.plyId)), s = [...et(), ...(h?.edges ?? []).filter((c) => o.has(c.id))];
    if (!s.length) return;
  ...
  function wt(e) {
    h = void 0, oe = void 0, H = void 0, a = { ...a, selectedId: void 0, focusedId: void 0, detailsHidden: !0 }, b.innerHTML = "", se(), S(), N(), y.dataset.empty = "true";
    const t = y.querySelector(".ply-empty");
    if (t) t.textContent = e;
    else {
      const o = document.createElement("p");
      o.className = "ply-empty", o.textContent = e, y.append(o);
    }
  ...
  const Fe = (e) => {
    ...
    }), f.querySelector("[data-fold-detail]").checked = a.foldDetail, f.querySelector("[data-hover-tooltips]").checked = a.hoverTooltips, f.querySelector("[data-show-legend]").checked = a.legendVisible, pe(), h && (V(), F(), j(), Y(me()))));
  }, Ye = (e) => {
