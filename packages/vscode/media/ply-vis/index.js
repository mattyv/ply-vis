...
}, me = (n) => Array.isArray(n) && n.every((r) => typeof r == "string"), ut = /* @__PURE__ */ new Set(["declared", "earned", "gap", "violation"]);
function ve(n) {
  if (n === null || typeof n == "boolean" || typeof n == "string" || typeof n == "number" && Number.isFinite(n)) return n;
  if (Array.isArray(n)) return Object.freeze(n.map(ve));
  if (L(n)) return Object.freeze(Object.fromEntries(Object.entries(n).map(([r, g]) => [r, ve(g)])));
...
function mt(n) {
  ...
  for (const [s, p] of Object.entries(n.elements)) {
    ...
    if (p.id !== s || typeof p.id != "string" || !p.id || typeof p.kind != "string" || !p.kind || typeof p.label != "string" || !p.label || typeof p.evidence.verdict != "string" || !me(p.evidence.statuses) || typeof p.evidence.reused != "boolean" || p.evidence.state !== void 0 && !ut.has(p.evidence.state) || !me(p.diagnosticIds) || p.parentId !== void 0 && typeof p.parentId != "string" || p.declaration !== void 0 && typeof p.declaration != "string" || p.limitations !== void 0 && !me(p.limitations)) throw new I(`Invalid element: ${s}`);
    const A = ve(p.evidence);
    f[s] = Object.freeze({ id: s, kind: p.kind, label: p.label, evidence: A, diagnosticIds: Object.freeze([...p.diagnosticIds]), ...p.parentId === void 0 ? {} : { parentId: p.parentId }, ...p.declaration === void 0 ? {} : { declaration: p.declaration }, ...p.limitations === void 0 ? {} : { limitations: Object.freeze([...p.limitations]) }, ...p.source === void 0 ? {} : { source: Fe(p.source) } });
  }
  ...
  for (const s of n.diagnostics) {
    ...
    w.add(s.id), h.push(Object.freeze({ id: s.id, code: s.code, severity: s.severity, message: s.message, ...s.elementId === void 0 ? {} : { elementId: s.elementId }, ...s.source === void 0 ? {} : { source: Fe(s.source) } }));
  }
...
const ht = /* @__PURE__ */ new Set(["script", "foreignobject", "iframe", "object", "embed", "audio", "video", "animate", "animatemotion", "animatetransform", "set"]), yt = /* @__PURE__ */ new Set(["href", "xlink:href", "src"]), gt = /^(?:[.#][A-Za-z_][\w-]*|[A-Za-z][\w-]*)(?:\s+(?:[.#][A-Za-z_][\w-]*|[A-Za-z][\w-]*))*$/, Ye = /^(?:none|#[0-9a-f]{3,8}|url\(#[A-Za-z_][\w:.-]*\))$/i, vt = {
  fill: Ye,
  stroke: Ye,
  "stroke-width": /^\d+(?:\.\d+)?$/,
...
}, bt = /^@media\s*\(\s*prefers-color-scheme\s*:\s*dark\s*\)\s*$/i;
function wt(n) {
  let r = n, g = "";
  for (; ; ) {
...
function _e(n, r = {}) {
  ...
  for (const m of [...f.querySelectorAll("*")]) {
    ...
    if (ht.has(m.localName.toLowerCase())) {
      m.remove();
      continue;
    }
    for (const b of [...m.attributes]) {
      ...
      (h.startsWith("on") || h === "style" || s || yt.has(h) && w !== "" && !w.startsWith("#")) && m.removeAttribute(b.name);
    }
  }
  return new XMLSerializer().serializeToString(f.documentElement);
}
const he = 1, zt = () => ({ post: (n) => window.parent.postMessage(n, "*") });
function xt(n) {
...
function Et(n) {
  if (typeof n != "object" || n === null) return !1;
  const r = n;
  return r.channel === "ply-vis" && r.version === 1 && (r.type === "artifact" && "envelope" in r || r.type === "restore-state" && xt(r.state));
}
const St = () => Object.freeze({ detailsHidden: !0, zoom: 1, panX: 0, panY: 0, foldDetail: !0, hoverTooltips: !0, optionsHidden: !1, overlays: Object.freeze({ earned: !0, gap: !0, violation: !0 }) }), Xe = (n, r) => Object.freeze({ ...n, ...r, overlays: Object.freeze({ ...n.overlays, ...r.overlays }) });
function kt(n, r, g = 0.5) {
...
const Lt = 500, Ot = 160, Ze = /* @__PURE__ */ new Set(["vscode-dark", "vscode-high-contrast"]), $t = /* @__PURE__ */ new Set(["vscode-light", "vscode-high-contrast-light"]);
function ye() {
  const n = typeof document < "u" ? document.body : void 0, r = n?.dataset.vscodeThemeKind;
  if (r !== void 0) return Ze.has(r);
  if (n) {
    for (const g of Ze) if (n.classList.contains(g)) return !0;
    for (const g of $t) if (n.classList.contains(g)) return !1;
  }
  return typeof window < "u" && typeof window.matchMedia == "function" && window.matchMedia("(prefers-color-scheme: dark)").matches;
}
const ge = (n, r) => `<button type="button" aria-label="${n}" title="${n}">${r}</button>`, Tt = `
  <section class="ply-vis" aria-label="Ply visual evidence viewer">
    <header class="ply-toolbar">
      <div class="ply-tools" role="group" aria-label="Canvas controls">
        ${ge("Zoom out", "−")}${ge("Zoom in", "+")}${ge("Fit canvas", "Fit")}
      </div>
...
function Dt(n, r, g = []) {
  n.innerHTML = Tt;
  const f = n.querySelector(".ply-vis"), m = f.querySelector(".ply-canvas"), b = f.querySelector(".ply-stage"), h = f.querySelector(".ply-tooltip"), w = f.querySelector(".ply-context-menu"), s = f.querySelector(".ply-inspector"), p = f.querySelector(".ply-inspector-toggle"), A = f.querySelector(".ply-workspace"), C = f.querySelector(".ply-status"), Z = f.querySelector(".ply-toolbar fieldset"), $ = f.querySelector(".ply-options"), z = f.querySelector(".ply-options-toggle"), D = f.querySelector(".ply-breadcrumbs"), R = f.querySelector(".ply-provenance");
  let a = St(), y, Q = /* @__PURE__ */ new Map(), ne, P = ye(), V;
  const ee = /* @__PURE__ */ new Set();
  let k, be = 0, S, W, oe;
  const we = () => r.post({ channel: "ply-vis", version: he, type: "persist-state", state: a }), O = (e, t = !0) => {
    a = Xe(a, e), t && we();
  }, B = () => {
    b.style.transform = `translate(${a.panX}px, ${a.panY}px) scale(${a.zoom})`;
  }, Pe = () => y ? Object.values(y.elements).filter((e) => !a.focusedId || e.id === a.focusedId || se(e, a.focusedId, y.elements)) : [];
  function F() {
  ...
  function Ue(e) {
    return e.run.tool.version === "render" || Object.values(e.elements).every((o) => xe(o) === "declared") ? { text: "Promises only — no run has checked this yet, so nothing here can ever be green." } : { text: `Showing a run completed ${new Date(e.run.completedAt).toLocaleString()}.`, title: `Run ${e.run.id}` };
  }
  function se(e, t, o) {
    let d = e.parentId;
    for (; d; ) {
  ...
  const Ee = () => a.foldDetail ? a.zoom < 0.8 ? 1 : a.zoom < 1.5 ? 2 : Number.POSITIVE_INFINITY : Number.POSITIVE_INFINITY;
  function Ke() {
...
`).filter(Boolean);
    ...
    if (s.append(T("Diagnostics", v.length ? v : ["No diagnostics supplied."])), s.append(Je(y.run)), o.source) {
  ...
  function E() {
    S && ke(S), q(), S = void 0, h.hidden = !0, h.replaceChildren();
  }
  function nt(e, t) {
    if (!y) return [];
  ...
  function Ae(e, t) {
    ...
    const i = 12, c = h.offsetWidth, l = h.offsetHeight, v = Math.max(d, o.width - c - d), u = Math.max(d, o.height - l - d), x = e - o.left + i, X = t - o.top + i, ue = X + l <= o.height - d ? X : t - o.top - l - i;
    h.style.left = `${Math.min(v, Math.max(d, x))}px`, h.style.top = `${Math.min(u, Math.max(d, ue))}px`;
  }
  function Ce(e, t, o) {
    ...
    if (i) {
      ...
      u.textContent = i.label, l.textContent = Se(i), h.replaceChildren(u, l);
    } else if (d) {
      const u = document.createElement("strong");
      u.textContent = d.label, l.textContent = nt(d, e).join(`
`), h.replaceChildren(u, l);
    } else
      l.textContent = c, h.replaceChildren(l);
    h.hidden = !1, et(e);
    const v = e.getBoundingClientRect();
    Ae(t ?? v.left + v.width / 2, o ?? v.bottom);
  }
  function Le(e, t, o) {
  ...
  function it(e) {
    ...
    if (d && i.push({ label: `Zoom into ${d.label}`, run: () => _(d.id) }), a.focusedId && i.push({ label: "Back to Workspace", run: () => _(void 0) }), y) {
      ...
      for (const v of d?.diagnosticIds ?? []) {
        const u = l.get(v)?.code;
        u && i.push({ label: `Explain ${u}`, run: () => r.post({ channel: "ply-vis", version: he, type: "explain", code: u }) });
      }
    }
    if (t && i.push({ label: "Explain a code…", run: () => r.post({ channel: "ply-vis", version: he, type: "explain-prompt" }) }), !i.length) return;
    e.preventDefault(), E();
    ...
    w.hidden = !1, ot(w, e.clientX, e.clientY), de(0);
  }
  w.addEventListener("keydown", (e) => {
    ...
    const o = Oe().indexOf(document.activeElement);
    e.key === "ArrowDown" ? (e.preventDefault(), de(o + 1)) : e.key === "ArrowUp" && (e.preventDefault(), de(o - 1));
  }), m.addEventListener("contextmenu", it);
  const $e = (e) => {
  ...
  function M() {
    ...
    for (const i of e) {
      ...
      if (v) {
        i.removeAttribute("hidden"), i.setAttribute("role", "button"), i.setAttribute("aria-label", Se(v)), i.classList.toggle("is-selected", v.id === a.selectedId);
        continue;
      }
      if (!l) {
      ...
      const u = xe(l), x = u === "declared" || a.overlays[u], X = t ? se(t, l.id, y.elements) : !1, ue = !a.focusedId || l.id === a.focusedId || se(l, a.focusedId, y.elements) || X, ft = X || Ge(l) <= Ee();
      i.toggleAttribute("hidden", !ue || !ft || !x && !X);
      const pt = [l.evidence.verdict, ...l.evidence.statuses].filter(Boolean).join(", ") || "declared";
      i.setAttribute("role", "button"), i.setAttribute("aria-label", `${l.kind}: ${l.label}; ${pt}`), i.dataset.state = u, i.classList.toggle("is-selected", l.id === a.selectedId), i === S && (i.hasAttribute("hidden") || !i.isConnected) && E();
    }
    st();
  ...
  function st() {
    ...
    for (const i of [...e.children]) {
      ...
      kt(d, l) || (i.setAttribute("hidden", ""), i.setAttribute("data-ply-focus-hidden", ""));
    }
  }
  function rt(e) {
    ...
    Z.hidden = !t, D.hidden = !t, p.hidden = !t, t || Ie(!0, !1), F(), E(), N(), V = void 0, ae(e.svg), m.dataset.empty = "false";
    const o = m.querySelector(".ply-empty");
    o && o.remove(), B(), M(), U(re());
    const d = Ue(e);
    R.textContent = d.text, d.title ? R.title = d.title : R.removeAttribute("title"), C.textContent = "", typeof window.requestAnimationFrame == "function" && window.requestAnimationFrame(() => fe(!1));
  }
  function ae(e) {
    b.innerHTML = e;
  ...
  function dt(e) {
    ...
    for (const d of b.querySelectorAll("[data-element-id], [data-ply-id]")) {
      ...
      try {
        ...
        ], { duration: Ot, easing: "ease-out", composite: "add" }), !(x.effect instanceof KeyframeEffect) || x.effect.composite !== "add") {
          x.cancel();
          continue;
        }
      } catch {
  ...
  function Me(e) {
    return Object.freeze({
      ...e,
      svg: _e(e.svg, { prefersDark: P }),
      folded: Object.freeze(e.folded.map((t) => Object.freeze({ depth: t.depth, svg: _e(t.svg, { prefersDark: P }) })))
    });
  }
  function ce(e) {
    try {
      const t = mt(e);
      return ne = t, rt(Me(t)), delete f.dataset.error, !0;
    } catch (t) {
  ...
  function _(e) {
    e && !y?.elements[e] || (e && !y.elements[e].parentId && (e = void 0), E(), N(), O({ focusedId: e, selectedId: e, detailsHidden: !e }), F(), M(), U(e ? y?.elements[e] : void 0), fe());
  }
  function lt() {
  ...
  function le(e, t) {
    E(), N(), Y(), t ??= lt(), O(Ct(a, Math.min(4, Math.max(0.2, e)), t)), B(), M(), C.textContent = `Zoom ${Math.round(a.zoom * 100)}%`;
  }
  function fe(e = !0) {
    Y();
    ...
    O(At({ width: m.clientWidth, height: m.clientHeight }, l)), B(), M();
  }
  f.querySelector('[aria-label="Zoom in"]').addEventListener("click", () => le(a.zoom * 1.2)), f.querySelector('[aria-label="Zoom out"]').addEventListener("click", () => le(a.zoom / 1.2)), f.querySelector('[aria-label="Fit canvas"]').addEventListener("click", () => fe()), p.addEventListener("click", () => Ie(!a.detailsHidden)), f.querySelectorAll("[data-overlay]").forEach((e) => e.addEventListener("change", () => {
    ...
    t === S && !h.hidden ? Ae(e.clientX, e.clientY) : Le(t, e.clientX, e.clientY);
    ...
    le(a.zoom * Math.exp(-e.deltaY * 2e-3), { x: e.clientX - t.left, y: e.clientY - t.top });
  ...
  const pe = () => {
    k && (k.moved && (be = performance.now() + 250, we()), k = void 0, m.classList.remove("is-panning"));
  };
  m.addEventListener("pointerup", pe), m.addEventListener("pointercancel", pe), m.addEventListener("lostpointercapture", pe), m.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && !h.hidden) {
      e.preventDefault(), E();
      return;
    }
    const t = [...b.querySelectorAll("[data-element-id], [data-ply-id]")], o = new Set(t.filter((c) => !c.hasAttribute("hidden") && J(c)).map((c) => c.dataset.elementId ?? c.dataset.plyId)), d = [...Pe(), ...(y?.edges ?? []).filter((c) => o.has(c.id))];
    if (!d.length) return;
  ...
  const Ne = (e) => {
    Et(e.data) && (e.data.type === "artifact" ? ce(e.data.envelope) : (a = Xe(a, e.data.state), f.querySelectorAll("[data-overlay]").forEach((t) => {
      t.checked = a.overlays[t.dataset.overlay];
    }), f.querySelector("[data-fold-detail]").checked = a.foldDetail, f.querySelector("[data-hover-tooltips]").checked = a.hoverTooltips, ie(), y && (F(), B(), M(), U(re()))));
  }, je = (e) => {
    C.textContent = `Viewer error: ${e}`, r.post({ channel: "ply-vis", version: 1, type: "error", message: e });
  }, qe = (e) => je(e.message || "Unknown runtime error"), He = (e) => je(e.reason instanceof Error ? e.reason.message : String(e.reason));
  window.addEventListener("message", Ne), window.addEventListener("error", qe), window.addEventListener("unhandledrejection", He);
  const Re = typeof MutationObserver == "function" ? new MutationObserver(() => ze(ye())) : void 0;
  Re?.observe(document.body, { attributes: !0, attributeFilter: ["class", "data-vscode-theme-kind"] });
  const Ve = typeof window.matchMedia == "function" ? window.matchMedia("(prefers-color-scheme: dark)") : void 0, Be = () => ze(ye());
  Ve?.addEventListener("change", Be), z.addEventListener("click", () => We(!a.optionsHidden)), F(), ie();
  for (const e of g) ce(e);
  return r.post({ channel: "ply-vis", version: 1, type: "ready" }), g.length || r.post({ channel: "ply-vis", version: 1, type: "request-artifact" }), { load: ce, getState: () => a, destroy: () => {
    q(), Y(), window.removeEventListener("message", Ne), window.removeEventListener("error", qe), window.removeEventListener("unhandledrejection", He), window.removeEventListener("pointerdown", $e), Re?.disconnect(), Ve?.removeEventListener("change", Be), n.replaceChildren();
  } };
}
const Nt = "default-src 'none'; img-src 'none'; style-src 'self'; script-src 'self'; font-src 'self'; connect-src 'none'; object-src 'none'; frame-src 'none'; base-uri 'none'; form-action 'none'";
export {
  Nt as CONTENT_SECURITY_POLICY,
  I as EnvelopeError,
  he as HOST_PROTOCOL_VERSION,
  Mt as PROTOCOL_VERSION,
  St as initialViewState,
  Et as isHostResponse,
  Dt as mountViewer,
  mt as parseEnvelope,
  _e as sanitizeSvg,
  Xe as updateViewState,
  zt as windowHostBridge
};
